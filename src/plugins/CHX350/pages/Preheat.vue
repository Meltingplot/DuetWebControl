		...
		<div class="grid">
			...
			<div class="side">
				<div class="chx-card card">
					...
					<p>{{ runner.configured.value ? $t("plugins.CHX350.preheat.macroHint") : $t("plugins.CHX350.generic.notConfigured") }}</p>
					<v-btn color="secondary" size="x-large" class="chx-btn" block :disabled="!runner.configured.value || uiFrozen || state.printing.value" :loading="runner.busy.value" @click="runner.run()">
...
async function allOff() {
	...
	try {
		...
		await machineStore.sendCode(codes.join("\n"), false, false);
	} finally {
